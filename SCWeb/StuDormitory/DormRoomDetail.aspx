<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DormRoomDetail.aspx.cs" Inherits="SCWeb.StuDormitory.DormRoomDetail" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
                ...
                <div class="date_place" style="margin: 5px 0px;">
                    ...
                    <div class="answer_tool fr clearfix">
                        ...
                        <div class="setting fl pr ml10" {{if IsLeader==0}}style="display:none;"{{/if}}>
                            ...
                            <div class="settings_none">
                                <em onclick="EditDormActivity(${Id});">修改</em>
                                <em onclick="DelDormActivity(${Id});">删除</em>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="center pt10 clearfix" id="divacco_right">
                        ...
                        <div class="tab_wrap none">
                            <div class="clearfix tools">
                               ...
                               <div class="fr tools_right">
                                    <a id="a_ExitBtn" href="javascript:;" class="tui" style="display:none;">我要退舍</a>                                    
                               </div>
                            </div>
            ...
            <div class="wrap_right fr">
                ...
                <div class="search_wrap pr">
                    <input id="txt_CurName" type="text" class="search mt10" placeholder="搜本宿舍话题">
            ...
            <div class="wrap">
                <div class="noacco clearfix">
                    ...
                    <div class="fl" style="margin-left:30px;">                        
                        <p id="p_nullWord"></p>
                        <a id="a_nullWord" href="/StuDormitory/AllAssociateManage.aspx" class="MySpeak bggreen"></a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $(function () {
            ...
            if("<%=UserInfo.UserType%>"=="2"){
                $("#p_nullWord").html('赶紧去加入宿舍吧，小伙伴都在等着你呢');
                $("#a_nullWord").html('去加入宿舍 >');
            } else {
                $("#p_nullWord").html('想拥有自己的宿舍？');
                $("#a_nullWord").html('去创建宿舍 >');
            }
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.result.errNum.toString() == "0") {
                        ...
                        ChangeBackPic();                        
                        LoadFunc(tabindex);
                        GetNewRoomStuListData();
                        GetNewestNew(0, CurRoomID);
                        TabChange();
                    }
                    else {
        ...
        function SetDisplayOrHide(model) { //设置显示隐藏
            if (model != '') {
                ...
                var $div_btnManage = $("#div_btnManage"); //管理宿舍按钮/申请加入按钮
                var $div_btnPicURL = $("#div_btnPicURL"); //更换背景按钮
                var $div_wrapsbtn = $("#div_wrapsbtn"); //右侧操作按钮
                var $div_TeachPlay = $("#div_TeachPlay"); //指导操作宿舍图片
                var $a_ExitBtn = $("#a_ExitBtn"); //退舍按钮
                var $a_Apply = $("#a_Apply");  //申请提示
                if (model.IsLeader == 1) {
                    $div_btnManage.show();
                    $div_btnManage.append('<i class="iconfont icon-settings"></i>管理宿舍');
                    $div_btnManage.click(function () {
                        JumpToPage(2, model.Id);//跳转宿舍管理页面                        
                    });
                    $div_btnPicURL.show();
                    $div_wrapsbtn.show();
                    ...
                    if (model.ApplyType==0&&model.ExamApplyCount > 0) {
                        ...
                        $a_Apply.click(function () {
                            JumpToPage(3, model.Id);//跳转宿舍管理页面并且默认选中成员审批选项卡 
                        });
                    } else {
                        $a_Apply.hide();
                    }
                    ...
                    if (model.IsMember == 1) {
                        ...
                        $a_ExitBtn.click(function () {
                            OpenIFrameWindow('申请退出', '/StuDormitory/AddAssoApply.aspx?itemid=0&relid=' + CurRoomID + '&astype=' + model.ApplyType + '&type=2', '480px', '400px');
                        });
                    } else {
                        if ("<%=UserInfo.UserType%>" == "2") {
                            $div_btnManage.show();
                            ...
                            $div_btnManage.click(function () {
                                OpenIFrameWindow('申请加入', '/StuDormitory/AddAssoApply.aspx?itemid=0&relid=' + relid + '&astype=' + astype + '&type=1', '480px', '400px');
                            });
                        }                        
                    }
                    $div_btnPicURL.hide();
        ...
        function SearchCondition() {
            ...
            LoadFunc($('#ul_RoomTab').find("li.selected").index());
        }
        function LoadFunc(tabindex) {
            SetUploadPicBtn(tabindex);
            if (tabindex == 0) {
                $txt_CurName.attr('placeholder', '搜本宿舍话题');
                GetComNewInfoData(1, 10);
            } else if (tabindex == 1) {
                $txt_CurName.attr('placeholder', '搜本宿舍活动');
                GetDormActivityData(1, 10);
            } else if (tabindex == 2) {
                $txt_CurName.attr('placeholder', '搜本宿舍相册');
                $aAlbum_TwoCrumbs = $("#aAlbum_TwoCrumbs");
                $aAlbum_TwoCrumbs.hide();
                GetComAlbumData(1, 10);
            } else if (tabindex == 3) {
                $txt_CurName.attr('placeholder', '搜本宿舍成员');
        ...
        //加载宿舍活动
        function GetDormActivityData(startIndex, pageSize) {
            ...
            pageNum = (startIndex - 1) * pageSize + 1;
            var parmaArray = {
                PageName: "/StuDormitory/Dorm_ActivityHandler.ashx",
                Func: "GetDormActivityDataPage",
                RoomId: CurRoomID,
                Name: CurNameVal,
                LoginUID: "<%=UserInfo.UniqueNo%>",
                PageIndex: startIndex,
                ...
                success: function (json) {
                    if (json.result.errNum.toString() == "0") {
                        ...
                        laypage({
                            ...
                            jump: function (obj, first) { //触发分页后的回调
                                if (!first) { //点击跳页触发函数自身，并传递当前页：obj.curr
                                    GetDormActivityData(obj.curr, pageSize)
                                }
                            }
                        });
                    }
        ...
        //加载最新加入成员
        function GetNewRoomStuListData() {
        ...
        function JumpToPage(type, itemid, newtype) {
            ...
            if (type == 0) {  //跳转帖子详情页面
                window.location.href = '/CommonPage/NewInfoDetail.aspx?type=2&newtype=' + newtype + '&itemid=' + itemid + '&nav=' + navid;
            } else if (type == 1) { //跳转活动详情页面
                window.location.href = '/StuDormitory/RoomActivityDetail.aspx?itemid=' + itemid + '&nav=' + navid;
            } else if (type == 2) { //跳转宿舍管理页面
                window.location.href = '/StuDormitory/SingleRoomManage.aspx?itemid=' + itemid + '&nav=' + navid;
            } else if (type == 3) { //跳转宿舍管理页面并且默认选中成员审批选项卡 
                window.location.href = '/StuDormitory/SingleRoomManage.aspx?itemid=' + itemid + '&nav=' + navid + "&tab=3";
            }
        }
        function PagePopWindow(type) {
            if (type == 0) { //发布话题
                OpenIFrameWindow('发布话题', '/CommonPage/EditComNewInfo.aspx?type=2&newtype=0&itemid=0&relid=' + CurRoomID, '730px', '600px');
            } else if (type == 1) { //发起活动
                OpenIFrameWindow('发起活动', '/StuDormitory/EditDormActivity.aspx?itemid=0&relid=' + CurRoomID, '900px', '540px');
            } else if (type == 2) { //创建相册
                OpenIFrameWindow('创建相册', '/CommonPage/EditComAlbum.aspx?type=2&itemid=0&relid=' + CurRoomID, '480px', '400px');
            } else if (type == 3) { //上传照片
                OpenIFrameWindow('上传照片', '/CommonPage/UploadAlbumPic.aspx?type=2&relid=' + CurRoomID, '900px', '660px');
            } else if (type == 4) { //添加成员
                OpenIFrameWindow('添加成员', '/StuDormitory/AddAssoMember.aspx?relid=' + CurRoomID + '&tab=3', '900px', '660px');
            }
        }
        function EditDormActivity(itemid) {
            OpenIFrameWindow('编辑活动', '/StuDormitory/EditDormActivity.aspx?itemid=' + itemid, '900px', '540px');
        }
        function DelDormActivity(itemid) {
        ...
        function ChangeBackPic() {            
            WebUploader.create({
                pick: '#filePicker',
                formData: { Func: "Upload_DormHomePic" },
                accept: {
                    title: 'Images',
