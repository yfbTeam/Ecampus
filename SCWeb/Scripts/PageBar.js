//生成页码条方法（方法对象,页码条容器，当前页码，总页数，页码组容量，总行数,全选）
function makePageBar(jsMethodName, pageContainer, pgIndex, pgCount, gpSize, roCount) {
    ...
    //生成统计数据
    //var pagingInfo = " 共" + pgIndex + "/" + pgCount + " 共" + roCount + "条";
    var pagingInfo = '共' + pgCount + '页，当前第' + pgIndex + '页';
    var pagingInfo1 = document.createElement("div");
    pagingInfo1.innerHTML = pagingInfo;
    pagingInfo1.className = 'com_curr fl';
    pageContainer.innerHTML = '';
    //生成首页
    //创建li标签
    var page_right = document.createElement('div');
    page_right.className = 'page_size fr';
    pageContainer.appendChild(page_right);
    ...
    page_right.appendChild(homePage);
    //生成 上一个页码组 按钮 
    ...
    page_right.appendChild(pagePrev);
    ...
    page_right.appendChild(pageNext);
    ...
    page_right.appendChild(endPage);
